...
init() ->
    ...
    %% set pin directions
    i2c:write(?I2C_CHANNEL, ?ADDR, {?IODIR, 16#f}, 2),
    %% reset pin value
    i2c:write(?I2C_CHANNEL, ?ADDR, {?GPIO, 0}, 2).
...
start(Count) when Count < 16 ->
    %% turn on the led
    i2c:write(?I2C_CHANNEL, ?ADDR, {?GPIO, Count bsl 4}, 2),
    timer:sleep(1000),
    
    %% read DIP switches
    i2c:write(?I2C_CHANNEL, ?ADDR, {?GPIO}, 1),
    timer:sleep(10),
